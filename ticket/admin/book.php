...
<body>
    ...
    <?php require '../assets/partials/_stud-header.php';?>
        ...
        <?php
            ...
            if(isset($_POST["view"]) && isset($_POST["university"])){

                $uni = $_POST["university"];
                
            $date = date('Y-m-d');
            $resultSql = "SELECT * FROM `events` WHERE university = $uni AND eventDate >= $date ORDER BY date_created DESC";
            ...
            if(!mysqli_num_rows($resultSqlResult)){ ?>
                ...
                <div class="container mt-4">
                    <div id="noCustomers" class="alert alert-dark " role="alert">
                        ...
                        <p class="fw-light">No event has been added for this university!</p>
                        <hr>
                        ...
                        <tr>
                            ...
                            <td>
                                            ...
                                            <button class="button delete-button" data-bs-toggle="modal" data-bs-target="#deleteModal" data-id="<?php 
                                                echo $id;?>" data-uni="<?php echo $uni; ?>" >Book Now</button>
                            </td>
                        </tr>
                        <?php 
                        }
    ...
    <div class="modal fade" id="addModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        ...
        <div class="modal-dialog modal-dialog-centered">
            ...
            <div class="modal-body">
                ...
                <p>
                    Do you really want to book this event? <strong>This process cannot be undone.</strong>
                </p>
                <!-- Needed to pass id -->
                <form action="<?php echo $_SERVER['REQUEST_URI']; ?>" id="delete-form"  method="POST">
                    <input id="delete-id" type="hidden" name="id">
		    <input id="delete-id" type="hidden" name="uni">
                    
                </form>
            </div>
            <div class="modal-footer d-flex justify-content-center">
                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
                <button type="submit" form="delete-form" name="book" class="btn btn-danger">Book</button>
            </div>
            </div>
        </div>
    </div>
 ...
 <?php
        ...
        */
        ...
        {
            if(isset($_POST["book"])) // Checks if submit buttons is clicked
            {
		...
		$uni = $_POST["uni"];
                ...
                $uni_exists = exist_booking($conn, $userID, $eventID); //calling exist_booking to check whether the booking is exixsting or not
                $uni_added = false;
			 ...
			 $sql = "SELECT * FROM `events` WHERE id = $eventID ";
                               ...
                               {
                                  $ticketAvailable = $rows['ticketAvailable'];
                                
                               }
			       if($ticketAvailable == 0){
				      ?>
				      <script>
                                           window.alert('Tickets for this event are sold out');
					   window.location.href="studDash.php";
				      </script>
                                    <?php				       
			       }
			       
			       $s = "SELECT * FROM `events` WHERE id = $eventID ";
                               $sres = mysqli_query($conn, $s);
                               while($rows1 = mysqli_fetch_assoc($sres))
                               {
                                  $uni = $rows1['university'];
                                
                               }
                    
                    $sql = "INSERT INTO `bookings`(`userID`, `eventID`, `university`, `booking_created`) VALUES ('$userID','$eventID','$uni', '$date')"; //Insert record to database

                    $result = mysqli_query($conn, $sql);

                    if($result)
			    // updating ticket availabilty after user successfully booked.
		              

		         $tickets = $ticketAvailable-1;
			$updateSql = "UPDATE `events` SET `ticketAvailable` =  $tickets WHERE `events`.`id` = $eventID;";
                ...
                if($uni_added)
                {
                    ...
                    <strong>Successful!</strong> Booking Information Added
